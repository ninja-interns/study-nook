...
func GetAllUsers(ctx context.Context) ([]*schema.User, error) {
	...
	err := pgxscan.Select(ctx, initializedb.Conn, &userList, query)
	if err != nil {
		return userList, err
	}
	return userList, nil
...
func AddUser(ctx context.Context, user *schema.User) error {
	...
	_, err := initializedb.Conn.Exec(ctx, query, user.ID, user.Email, user.PasswordHash, user.Name, user.Username, user.IsVerified, user.Token)
	if err != nil {
		return err
	}
	return nil
...
func GetUserByID(ctx context.Context, userID string) (*schema.User, error) {
	...
	user := &schema.User{}
	err := initializedb.Conn.QueryRow(ctx, query, userID).Scan(&user.ID, &user.Email, &user.Name, &user.Username, &user.IsVerified, &user.Token)
	if err != nil {
		return user, err
	}
...
func UpdateUser(ctx context.Context, userID string, userData *schema.User) error {
	...
	res, err := initializedb.Conn.Exec(ctx, query, userData.Email, userData.Name, userData.Username, userData.PasswordHash, userData.IsVerified, userData.Token, userID)
...
// UpdateUserExceptPassword updates the user with given ID
func UpdateUserExceptPassword(ctx context.Context, userID string, userData *schema.User) error {
	query := `UPDATE users SET email=$1, name=$2, username=$3, is_verified=$4, token=$5 WHERE id=$6`
	res, err := initializedb.Conn.Exec(ctx, query, userData.Email, userData.Name, userData.Username, userData.IsVerified, userData.Token, userID)
	if err != nil || res.RowsAffected() == 0 {
		fmt.Println(err.Error())
		return err
	}
...
func DeleteUser(ctx context.Context, userID string) error {
	...
	res, err := initializedb.Conn.Exec(ctx, query, userID)
	if err != nil || res.RowsAffected() == 0 {
		return err
	}
	return nil
