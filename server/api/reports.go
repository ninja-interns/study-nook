...
func (c *Controller) SubmitReports(w http.ResponseWriter, r *http.Request, u *studynook.User) {
	...
	if check {
		...
		err = emails.Send(*c.Emailer, "studynookapp@gmail.com", "Submission Report", "reportSubmission.html", map[string]string{"id": submission_id, "username": report.Username, "message": report.Message, "date": report.Date})
		if err != nil {
			fmt.Println(err)
			return
		}
